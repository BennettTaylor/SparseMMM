...
// gcc -O1 serial_sparse_mmm.c -lrt -o serial_spare_mmm
/*****************************************************************************/
...
  printf("Matrix size, Density, CSR time (cycles), COO time (cycles)\n");
  for (int i = 0; i < NUM_SIZES; i++) {
    for (int j = 0; j < NUM_DENSITIES; j++) {
      printf("%d, %f, ", matrix_sizes[i], matrix_densities[j]);
      for (int opt = 0; opt < OPTIONS; opt++) {
        if (opt != 0) {
          printf(", ");
        }
        printf("%ld", (long int)((double)(CPNS) * 1.0e9 * time_stamp[opt][i][j]));
      }
      printf("\n");
    }
  }
