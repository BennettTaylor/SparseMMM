...
// gcc -O1 parallel_sparse_mmm.c -lrt -o parallel_sparse_mmm
/*****************************************************************************/
...
void mmm_csr_parallel(coo_matrix_ptr a, coo_matrix_ptr b, coo_matrix_ptr c);
...
/* Thread-safe CSR matrix multiplication with OpenMP */
void mmm_csr_parallel(csr_matrix_ptr a, csr_matrix_ptr b, csr_matrix_ptr c) 
{
    // Initialize output matrix with conservative capacity estimate
    size_t capacity = a->len * b->len; // Upper bound (very sparse matrices won't need this much)
    c->values = realloc(c->values, capacity * sizeof(data_t));
    c->columns = realloc(c->columns, capacity * sizeof(int));
    c->nnz = 0;
    c->row_indices[0] = 0;

    // Pre-transpose matrix b for better memory access patterns
    csr_matrix_ptr b_transposed = transpose_csr(b);
    if (!b_transposed) {
        fprintf(stderr, "Failed to transpose matrix B\n");
        return;
    }

    // Thread-local storage for temporary arrays
    data_t** thread_temp_values;
    int** thread_temp_cols;
    int* thread_nnz;
    ...
    #pragma omp parallel
    {
        ...
        #pragma omp single
        {
            thread_temp_values = malloc(num_threads * sizeof(data_t*));
            thread_temp_cols = malloc(num_threads * sizeof(int*));
            thread_nnz = calloc(num_threads, sizeof(int));
            
            for (int t = 0; t < num_threads; t++) {
                thread_temp_values[t] = calloc(a->len, sizeof(data_t));
                thread_temp_cols[t] = malloc(a->len * sizeof(int));
            }
        }

        // Parallelize over rows of matrix a
        #pragma omp for schedule(dynamic, 16) nowait
        for (int i = 0; i < a->len; i++) {
            data_t* temp = thread_temp_values[thread_id];
            int* temp_idx = thread_temp_cols[thread_id];
            int nnz = 0;
            ...
            for (int j = a->row_indices[i]; j < a->row_indices[i+1]; j++) {
                ...
                for (int k = b_transposed->row_indices[a_col]; 
                     k < b_transposed->row_indices[a_col+1]; k++) {
                    int b_col = b_transposed->columns[k];
                    
                    if (temp[b_col] == 0) {
                        temp_idx[nnz++] = b_col;
                    }
                    temp[b_col] += a_val * b_transposed->values[k];
                }
            }
            ...
            // Reserve space in output matrix (critical section)
            int start_pos;
            #pragma omp critical
            {
                start_pos = c->nnz;
            ...
            for (int j = 0; j < nnz; j++) {
                int col = temp_idx[j];
                c->values[start_pos + j] = temp[col];
                c->columns[start_pos + j] = col;
                temp[col] = 0;  // Reset for next row
            }
        }

        // Clean up thread-local storage
        free(thread_temp_values[thread_id]);
        free(thread_temp_cols[thread_id]);
    }
    ...
    // Clean up
    free(thread_temp_values);
    free(thread_temp_cols);
    free(thread_nnz);
    free_csr_matrix(b_transposed);
}
